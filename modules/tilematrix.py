...
class MetaTileMatrix(TileMatrix):
    ...
    def tilesize_for_zoom(self, zoom):
        ...
        try:
            ...
            print "Zoom (%s) must be an integer." %(zoom)
            sys.exit(0)
        #tile_wesize = float(round(self.tilematrix.tilesize_for_zoom(zoom)[0] * 
        #    float(self.metatiles), ROUND))
        #if tile_wesize > self.wesize:
        #    tile_wesize = self.wesize
        #tile_nssize = float(round(self.tilematrix.tilesize_for_zoom(zoom)[1] * 
        #    float(self.metatiles), ROUND))
